 ...
 * $mail->setCc("XXXX"); //设置抄送
 * $mail->setBcc("XXXXX"); //设置秘密抄送
...
class Mail_Class {
    ...
    protected function getCommand() {
        ...
        //设置发件人
        $command[] = array("MAIL FROM: <{$this->_from['mail']}>\r\n", 250);
        $header = "FROM: {$this->_from['name']}<{$this->_from['mail']}>\r\n";

        //设置收件人        
        if (!empty($this->_to)) {
            $count = count($this->_to);
            if ($count == 1) {
                $command[] = array("RCPT TO: <{$this->_to[0]['mail']}>\r\n", 250);
                $header .= "TO: {$this->_to[0]['name']}<{$this->_to[0]['mail']}>\r\n";
            } else {
                for ($i = 0; $i < $count; $i++) {
                    $command[] = array("RCPT TO: <{$this->_to[$i]['mail']}>\r\n", 250);
                    if ($i == 0) {
                        $header .= "TO: {$this->_to[$i]['name']}<{$this->_to[$i]['mail']}>";
                    } elseif ($i + 1 == $count) {
                        $header .= ",{$this->_to[$i]['name']}<{$this->_to[$i]['mail']}>\r\n";
                    } else {
                        $header .= ",{$this->_to[$i]['name']}<{$this->_to[$i]['mail']}>";
                    }
                }
        ...
        if (!empty($this->_cc)) {
            ...
            if ($count == 1) {
                $command[] = array("RCPT TO: <{$this->_cc[0]['mail']}>\r\n", 250);
                $header .= "CC: {$this->_cc[0]['name']}<{$this->_cc[0]['mail']}>\r\n";
            } else {
                for ($i = 0; $i < $count; $i++) {
                    $command[] = array("RCPT TO: <{$this->_cc[$i]['mail']}>\r\n", 250);
                    if ($i == 0) {
                        $header .= "CC: {$this->_cc[$i]['name']}<{$this->_cc[$i]['mail']}>";
                    } elseif ($i + 1 == $count) {
                        $header .= ",{$this->_cc[$i]['name']}<{$this->_cc[$i]['mail']}>\r\n";
                    } else {
                        $header .= ",{$this->_cc[$i]['name']}<{$this->_cc[$i]['mail']}>";
                    }
                }
        ...
        if (!empty($this->_bcc)) {
            ...
            if ($count == 1) {
                $command[] = array("RCPT TO: <{$this->_bcc[0]['mail']}>\r\n", 250);
                $header .= "BCC: {$this->_bcc[0]['name']}<{$this->_bcc[0]['mail']}>\r\n";
            } else {
                for ($i = 0; $i < $count; $i++) {
                    $command[] = array("RCPT TO: <{$this->_bcc[$i]['mail']}>\r\n", 250);
                    if ($i == 0) {
                        $header .= "BCC: {$this->_bcc[$i]['name']}<{$this->_bcc[$i]['mail']}>";
                    } elseif ($i + 1 == $count) {
                        $header .= ",{$this->_bcc[$i]['name']}<{$this->_bcc[$i]['mail']}>\r\n";
                    } else {
                        $header .= ",{$this->_bcc[$i]['name']}<{$this->_bcc[$i]['mail']}>";
                    }
                }
